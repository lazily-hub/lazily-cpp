...
inline void pack_node_state(MsgPacker& p, const NodeState& s) {
  ...
  if (std::holds_alternative<NodeStatePayload>(s)) {
    ...
    p.i64(1);
    p.str("blob");
    pack_shm_blob_ref(p, std::get<NodeStateSharedBlob>(s).blob);
  } else {
    p.map_header(1);
    p.str("kind");
    p.i64(2);
...
inline NodeState unpack_node_state(MsgUnpacker& u) {
  ...
  for (uint32_t i = 0; i < n; ++i) {
    std::string_view k = u.read_str_view();
    if (k == "kind")
      kind = static_cast<int>(u.read_i64());
    else if (k == "bytes")
      bytes = u.read_bin();
    else if (k == "blob")
      blob = unpack_shm_blob_ref(u);
    else
      u.skip();
  }
...
inline void pack_ipc_value(MsgPacker& p, const IpcValue& v) {
  if (std::holds_alternative<IpcValueInline>(v)) {
    ...
    p.i64(1);
    p.str("blob");
    pack_shm_blob_ref(p, std::get<IpcValueSharedBlob>(v).blob);
  }
}
...
inline void pack_snapshot(MsgPacker& p, const Snapshot& s) {
  ...
  p.str("nodes");
  {
    ...
    for (auto& nd : s.nodes) {
      ...
      p.str(nd.type_tag);
      p.str("state");
      pack_node_state(p, nd.state);
      p.str("key");
      pack_optional_node_key(p, nd.key);
    }
  }
  p.str("edges");
  {
      ...
      for (uint32_t j = 0; j < m; ++j) {
        ...
        for (uint32_t f = 0; f < fn; ++f) {
          ...
          if (fk == "node")
            ...
            nd.state = unpack_node_state(u);
          else if (fk == "key")
            nd.key = unpack_optional_node_key(u);
          else
            u.skip();
        }
  ...
  p.str("ops");
  {
    ...
    for (auto& op : d.ops) {
      ...
      if (std::holds_alternative<DeltaOpCellSet>(op)) {
        ...
        p.str(v.type_tag);
        p.str("state");
        pack_node_state(p, v.state);
        p.str("key");
        pack_optional_node_key(p, v.key);
    ...
    if (k == "base_epoch")
      ...
      for (uint32_t j = 0; j < m; ++j) {
        ...
        for (uint32_t f = 0; f < fn; ++f) {
          std::string_view fk = u.read_str_view();
          if (fk == "op")
            op_kind = static_cast<int>(u.read_i64());
          else if (fk == "node")
            node = u.read_i64();
          else if (fk == "dependent")
            dependent = u.read_i64();
          else if (fk == "dependency")
            dependency = u.read_i64();
          else if (fk == "type_tag")
            type_tag = u.read_str();
          else if (fk == "state")
            state = unpack_node_state(u);
          else if (fk == "key")
            key = unpack_optional_node_key(u);
          else if (fk == "payload")
            payload = unpack_ipc_value(u);
          else
            u.skip();
        }
        switch (op_kind) {
        case 0:
          d.ops.push_back(DeltaOpCellSet{node, std::move(payload)});
          break;
        case 1:
          d.ops.push_back(DeltaOpSlotValue{node, std::move(payload)});
          break;
        case 2:
          d.ops.push_back(DeltaOpInvalidate{node});
          break;
        case 3:
          d.ops.push_back(
              DeltaOpNodeAdd{node, std::move(type_tag), std::move(state), std::move(key)});
          break;
        case 4:
          d.ops.push_back(DeltaOpNodeRemove{node});
          break;
        case 5:
          d.ops.push_back(DeltaOpEdgeAdd{dependent, dependency});
          break;
        case 6:
...
inline void pack_crdt_sync(MsgPacker& p, const CrdtSync& c) {
  ...
  p.str("ops");
  {
    ...
    for (auto& o : c.ops) {
      ...
      p.i64(o.node);
      p.str("key");
      pack_optional_node_key(p, o.key);
      p.str("stamp");
      pack_wire_stamp(p, o.stamp);
      p.str("state");
      pack_ipc_value(p, o.state);
    }
  }
}
  ...
  for (uint32_t i = 0; i < n; ++i) {
    ...
    if (k == "frontier") {
      ...
      for (uint32_t j = 0; j < m; ++j) {
        ...
        if (fn != 2)
          throw std::runtime_error("codec: StampFrontierEntry must be a 2-element array");
      ...
      for (uint32_t j = 0; j < m; ++j) {
        ...
        for (uint32_t f = 0; f < fn; ++f) {
          ...
          if (fk == "node")
            ...
            o.key = unpack_optional_node_key(u);
          else if (fk == "stamp")
            o.stamp = unpack_wire_stamp(u);
          else if (fk == "state")
            o.state = unpack_ipc_value(u);
          else
            u.skip();
        }
...
inline void pack_ipc_message(MsgPacker& p, const IpcMessage& m) {
  ...
  if (std::holds_alternative<IpcMessageSnapshot>(m)) {
    ...
    p.i64(0);
    p.str("value");
    pack_snapshot(p, std::get<IpcMessageSnapshot>(m).value);
  } else if (std::holds_alternative<IpcMessageDelta>(m)) {
    p.map_header(2);
    p.str("type");
    p.i64(1);
    p.str("value");
    pack_delta(p, std::get<IpcMessageDelta>(m).value);
  } else if (std::holds_alternative<IpcMessageCrdtSync>(m)) {
    p.map_header(2);
    p.str("type");
    p.i64(2);
    p.str("value");
    pack_crdt_sync(p, std::get<IpcMessageCrdtSync>(m).value);
  } else if (std::holds_alternative<IpcMessageResyncRequest>(m)) {
    p.map_header(2);
    p.str("type");
    p.i64(3);
    p.str("value");
    pack_resync_request(p, std::get<IpcMessageResyncRequest>(m).value);
  } else {
    p.map_header(2);
    p.str("type");
    p.i64(4);
    p.str("value");
    pack_outbox_ack(p, std::get<IpcMessageOutboxAck>(m).value);
  }
}
inline IpcMessage unpack_ipc_message(MsgUnpacker& u) {
  ...
  for (uint32_t i = 0; i < n; ++i) {
    ...
    if (k == "type") {
      ...
      switch (type) {
      case 0:
        return IpcMessageSnapshot{unpack_snapshot(u)};
      case 1:
        return IpcMessageDelta{unpack_delta(u)};
      case 2:
        return IpcMessageCrdtSync{unpack_crdt_sync(u)};
      case 3:
        return IpcMessageResyncRequest{unpack_resync_request(u)};
      case 4:
        return IpcMessageOutboxAck{unpack_outbox_ack(u)};
      default:
...
inline void pack_optional_node_key_positional(MsgPacker& p, const std::optional<NodeKey>& k) {
  if (k)
    p.str(k->path());
  else
    p.nil();
...
inline WireStamp unpack_wire_stamp_positional(MsgUnpacker& u) {
  ...
  for (uint32_t i = 4; i < n; ++i)
    u.skip(); // forward-compat extra fields
  return s;
}
...
inline void pack_snapshot_positional(MsgPacker& p, const Snapshot& s) {
  ...
  p.array_header(static_cast<uint32_t>(s.nodes.size()));
  for (auto& nd : s.nodes)
    pack_node_snapshot_positional(p, nd);
  p.array_header(static_cast<uint32_t>(s.edges.size()));
  for (auto& e : s.edges)
    pack_edge_snapshot_positional(p, e);
  p.array_header(static_cast<uint32_t>(s.roots.size()));
  for (auto r : s.roots)
    p.i64(r);
}
...
inline void pack_stamp_frontier_entry_positional(MsgPacker& p, const StampFrontierEntry& e) {
  p.array_header(3);
...
inline StampFrontierEntry unpack_stamp_frontier_entry_positional(MsgUnpacker& u) {
  uint32_t n = u.read_array_header();
  if (n < 3) throw std::runtime_error("codec: StampFrontierEntry positional arity < 3");
  int64_t schema = u.read_i64();
...
inline void pack_crdt_sync_positional(MsgPacker& p, const CrdtSync& c) {
  ...
  p.array_header(static_cast<uint32_t>(c.frontier.size()));
  for (auto& e : c.frontier)
    pack_stamp_frontier_entry_positional(p, e);
  p.array_header(static_cast<uint32_t>(c.ops.size()));
  for (auto& o : c.ops)
    pack_crdt_op_positional(p, o);
}
...
inline IpcMessage unpack_ipc_message_positional(MsgUnpacker& u) {
  uint32_t n = u.read_array_header();
  if (n < 3) throw std::runtime_error("codec: IpcMessage positional arity < 3");
  int64_t codec_version = u.read_i64();
  if (codec_version != kPositionalCodecVersion)
    throw std::runtime_error("codec: positional codec version mismatch (incompatible envelope)");
...
inline IpcMessage decode(const std::vector<uint8_t>& b) { return decode(b.data(), b.size()); }
