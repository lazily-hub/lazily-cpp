...
struct CrdtTree<
    Tree, std::void_t<typename Tree::Value, typename Tree::VersionVector, typename Tree::Delta,
                      ...
                      decltype(std::declval<Tree&>().merge_from(std::declval<const Tree&>()))>>
    : std::integral_constant<
          bool,
          std::is_same<decltype(std::declval<const Tree&>().version_vector()),
                       typename Tree::VersionVector>::value &&
              std::is_same<decltype(std::declval<const Tree&>().delta_since(
                               std::declval<const typename Tree::VersionVector&>())),
              ...
              std::is_same<decltype(std::declval<Tree&>().merge_from(std::declval<const Tree&>())),
                           bool>::value> {};

template <typename Tree> inline constexpr bool is_crdt_tree_v = CrdtTree<Tree>::value;
