...
Env env(const char* key, std::uint64_t generation, std::uint64_t sequence, std::uint64_t stamped_at,
        Payload payload) {
...
void in_order_delivery_conflates_and_receipts() {
  ...
  REQUIRE(first.second == IngressAdmission::accepted(0), "first in-order envelope is Accepted");
  ...
  REQUIRE(second.second == IngressAdmission::conflated(1), "a second in-order envelope coalesces");
  REQUIRE(core.peek(Key("a")) == std::optional<Payload>(12),
          "the window is the merge of both payloads");
  REQUIRE(core.receipts(IngressReceiptChannel::Accepted).size() == 2, "two accepted receipts");
  REQUIRE(core.receipts(IngressReceiptChannel::Dropped).empty(), "no dropped receipts");
}

void reorder_buffers_then_flushes_in_one_invalidation() {
  SumCore core(default_policy());
  auto buffered = core.admit(env("a", 1, 2, 0, 4));
  REQUIRE(buffered.second == IngressAdmission::buffered(0), "an out-of-order envelope buffers");
  ...
  REQUIRE(buffered.first.scopes.size() == 1 &&
              buffered.first.scopes[0].second == lazily::IngressScopeChange::creation(),
  ...
  REQUIRE(again.first.empty(), "a second buffered envelope on a known scope is invisible");
  ...
  REQUIRE(core.peek(Key("a")) == std::optional<Payload>(7), "1 + 2 + 4 lands as ONE window");
...
void duplicates_are_dropped_after_delivery_and_while_buffered() {
  ...
  REQUIRE(core.peek(Key("a")) == std::optional<Payload>(1), "duplicates never reach the window");
}
...
void a_stale_generation_is_fenced_before_its_sequence_is_consulted() {
  ...
  REQUIRE(core.peek(Key("a")) == std::optional<Payload>(1), "a zombie payload never merges");
}
...
void a_newer_generation_hands_off_and_resets_the_sequence_space() {
  ...
  REQUIRE(core.admit(env("a", 2, 0, 0, 4)).second == IngressAdmission::generation_handoff(1, 2),
...
void a_handoff_that_buffers_still_reports_the_baseline_reset() {
  ...
  REQUIRE(!core.peek(Key("a")).has_value(), "the superseded window is discarded");
  const auto view = core.view(Key("a"));
  REQUIRE(view->generation == 2 && !view->delivered_through.has_value() && view->buffered == 1,
...
void an_expired_envelope_never_occupies_a_reorder_slot() {
  ...
  REQUIRE(!core.view(Key("a")).has_value(), "a refusal must not materialize a scope");
  REQUIRE(core.admit(env("a", 1, 3, 95, 1)).second == IngressAdmission::buffered(0),
...
void block_overflow_refuses_without_losing_the_window() {
  ...
  REQUIRE(core.view(Key("a"))->delivered_through == std::optional<std::uint64_t>(0),
          "Block does not advance the watermark");
  core.drain(Key("a"));
  REQUIRE(core.admit(env("a", 1, 1, 0, 9)).second == IngressAdmission::accepted(1),
...
void drop_oldest_restarts_the_window_at_the_incoming_op() {
  ...
  REQUIRE(core.admit(env("a", 1, 2, 0, 30)).second == IngressAdmission::accepted(2),
...
void drop_newest_keeps_the_window_and_receipts_the_drop() {
  ...
  REQUIRE(dropped.second == IngressAdmission::dropped(IngressDropReason::Backpressure),
          "DropNewest discards the incoming op");
  REQUIRE(dropped.first.dropped_receipts, "the drop is receipted");
  REQUIRE(core.peek(Key("a")) == std::optional<Payload>(5), "the accumulated window survives");
}
...
void readiness_derives_from_lifecycle_and_freshness() {
  ...
  policy.freshness_horizon = 10;
  SumCore core(policy);
  REQUIRE(core.readiness(Key("a")) == IngressReadiness::Unknown, "an unknown scope is Unknown");
  core.open(Key("a"), 1);
  ...
  REQUIRE(crossing.scopes.size() == 1 &&
              crossing.scopes[0].second == lazily::IngressScopeChange::readiness_only(),
          "crossing the horizon is a readiness-ONLY transition");
  REQUIRE(core.readiness(Key("a")) == IngressReadiness::Stale, "now stale");
  REQUIRE(core.tick(60).empty(), "a further tick inside the same readiness dirties nothing");
}
...
void reconnect_at_a_higher_generation_discards_the_stale_window() {
  ...
  for (const auto& entry : reconnected.first.scopes)
    if (entry.second.value && entry.second.authority) value_and_authority = true;
...
void errors_deepen_backoff_and_a_delivery_clears_it() {
  ...
  SumCore core(policy);
  core.open(Key("a"), 1);
  REQUIRE(!core.retry(Key("a")).has_value(), "a healthy scope has no backoff, not a zero one");

  core.fail(Key("a"), IngressError::TransportClosed);
  REQUIRE(core.retry(Key("a")) == std::optional<IngressRetry>(IngressRetry{1, 10, 0}),
          "the first error yields the base backoff");
  core.fail(Key("a"), IngressError::TransportClosed);
  REQUIRE(core.retry(Key("a"))->backoff == 20, "the backoff doubles");
  core.fail(Key("a"), IngressError::TransportClosed);
  REQUIRE(core.retry(Key("a"))->backoff == 25, "clamped to the ceiling, not doubled past it");
...
void a_reconnect_clears_the_error_streak_without_a_delivery() {
  ...
  for (const auto& entry : reconnected.first.scopes)
    if (entry.second.retry) retry_marked = true;
...
void closed_scopes_admit_nothing_and_claim_no_authority() {
  ...
  core.admit(env("a", 1, 0, 0, 1));
  core.close(Key("a"));
  REQUIRE(!core.authority(Key("a")).has_value(), "a closed scope claims no authority");
  REQUIRE(core.admit(env("a", 1, 1, 0, 1)).second ==
              IngressAdmission::dropped(IngressDropReason::ScopeClosed),
          "a closed scope admits nothing");
  core.open(Key("a"), 1);
  REQUIRE(core.admit(env("a", 1, 0, 0, 4)).second == IngressAdmission::accepted(0),
...
void a_schedule_offers_a_poll_interval_only_without_event_delivery() {
  REQUIRE(
      !IngressSchedule::for_kind(IngressTransportKind::EventChannel, 50).poll_interval.has_value(),
      "an event channel needs no poll");
  REQUIRE(
      !IngressSchedule::for_kind(IngressTransportKind::RpcTriggered, 50).poll_interval.has_value(),
      "an RPC trigger needs no poll");
  REQUIRE(IngressSchedule::for_kind(IngressTransportKind::BoundedPolling, 50).poll_interval ==
              std::optional<std::uint64_t>(50),
          "bounded polling carries its bound");
  REQUIRE(IngressSchedule::for_kind(IngressTransportKind::BoundedPolling, 0).poll_interval ==
...
void drain_is_a_value_only_transition_and_empty_drains_dirty_nothing() {
  ...
  REQUIRE(drained.first.scopes.size() == 1 &&
              drained.first.scopes[0].second == lazily::IngressScopeChange::value_only(),
          "a drain is a value-only transition");
  auto empty = core.drain(Key("a"));
  REQUIRE(!empty.second.has_value() && empty.first.empty(), "an empty drain invalidates nothing");
  REQUIRE(core.view(Key("a"))->delivered_through == std::optional<std::uint64_t>(0),
...
void out_of_order_arrival_converges_to_the_in_order_fold() {
  ...
  for (const auto& order : permutations) {
    ...
    REQUIRE(core.view(Key("a"))->delivered_through == std::optional<std::uint64_t>(3),
...
template <typename Cell, typename Cx> struct Shell {
  Cx ctx;
  Cell cell;
  Shell(IngressPolicy policy, IngressTransportKind kind, std::uint64_t poll_interval)
...
void readiness_authority_and_retry_are_derives(const char* flavour) {
  ...
  REQUIRE(s.cell.authority(s.ctx, Key("a")) == std::optional<IngressAuthority>(IngressAuthority{
                                                   3, std::optional<std::uint64_t>(0), 5}),
...
void a_buffered_envelope_reruns_no_effect(const char* flavour) {
  ...
  REQUIRE(runs == 1, std::string(flavour) + ": a buffered envelope invalidates nothing");
...
void a_tick_inside_the_horizon_reruns_no_readiness_effect(const char* flavour) {
  ...
  REQUIRE(runs == 1, std::string(flavour) + ": a tick inside the horizon is not a change");
  s.cell.tick(s.ctx, 500);
  REQUIRE(runs == 2, std::string(flavour) + ": crossing the horizon IS a change");
  s.graph().dispose_effect(effect);
}
...
void a_generation_handoff_lands_in_one_frontier_walk(const char* flavour) {
  ...
  REQUIRE(runs == 2, std::string(flavour) + ": one admission is ONE effect run, not one per "
                                            "dirtied reader kind");
  REQUIRE(seen.size() == 2 && seen[0].first == std::optional<Payload>(5) && seen[0].second == 1 &&
              seen[1].first == std::optional<Payload>(9) && seen[1].second == 2,
          std::string(flavour) + ": value and authority land together, never new/old");
  s.graph().dispose_effect(effect);
}
...
void receipt_channels_are_independent_readers(const char* flavour) {
  ...
  REQUIRE(s.cell.accepted(s.ctx).size() == 1 && s.cell.dropped(s.ctx).empty() &&
              s.cell.errors(s.ctx).empty(),
  ...
  REQUIRE(dropped.size() == 1 && dropped[0].outcome == IngressReceiptOutcome::dropped(
...
template <typename Cell, typename Cx>
void the_schedule_derives_from_the_transport_and_retunes_live(const char* flavour) {
  Shell<Cell, Cx> s(default_policy(), IngressTransportKind::EventChannel, 25);
  ...
  REQUIRE(s.cell.schedule(s.ctx).poll_interval == std::optional<std::uint64_t>(25),
          std::string(flavour) + ": the fallback bound was retained");
  s.cell.set_poll_interval(s.ctx, 200);
  REQUIRE(s.cell.schedule(s.ctx).poll_interval == std::optional<std::uint64_t>(200),
...
template <typename Cell, typename Cx>
void pump_admits_a_batch_and_requests_replay_for_a_surviving_gap(const char* flavour) {
  Shell<Cell, Cx> s(default_policy(), IngressTransportKind::EventChannel, 25);
  ...
  REQUIRE(transport.replays().size() == 1 && transport.replays()[0].first == "a" &&
              transport.replays()[0].second == (ReplayRequest{1, 1}),
          std::string(flavour) + ": the gap the algebra reports is the gap replayed");
...
void a_polling_transport_cannot_serve_a_replay(const char* flavour) {
  ...
  REQUIRE(transport.replays().empty(),
          std::string(flavour) + ": a bounded poll has no addressable history, so 'this gap will "
...
int main() {
  ...
  every_shell_behaviour<IngressCell<Key, Payload, Sum>, Context>("single-threaded");
  every_shell_behaviour<ThreadSafeIngressCell<Key, Payload, Sum>, ThreadSafeContext>("thread-safe");
  every_shell_behaviour<AsyncIngressCell<Key, Payload, Sum>, AsyncContext>("async");

  std::cout << "ingress: algebra invariants + 10 shell behaviours x 3 flavours OK" << std::endl;
  return 0;
}
