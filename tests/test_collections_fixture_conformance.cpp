...
JsonPtr fixture(const std::string& name) {
  return lazily_test::parse_json(lazily_test::spec_fixture_text("collections", name));
}
...
std::vector<std::pair<std::string, int>> keyed_values(const Json& value) {
  const auto order = string_array(lazily_test::json_member(value, "order"));
  const auto& values = lazily_test::json_member(value, "values");
  std::vector<std::pair<std::string, int>> result;
  for (const auto& key : order) {
    result.emplace_back(
        key, static_cast<int>(lazily_test::json_u64(lazily_test::json_member(values, key))));
  }
  return result;
}

template <typename Policy> void replay_merge_scenario(const Json& scenario) {
  const auto& flags = lazily_test::json_member(scenario, "flags");
  assert(Policy::commutative ==
         lazily_test::json_bool(lazily_test::json_member(flags, "commutative")));
  assert(Policy::idempotent ==
         lazily_test::json_bool(lazily_test::json_member(flags, "idempotent")));

  Context ctx;
  MergeCell<long, Policy> cell(
      ctx, static_cast<long>(lazily_test::json_member(scenario, "initial").as_int()));
  auto observed = ctx.computed<long>([&](Compute& compute) { return cell.get(compute); });
  (void)ctx.get(observed);

  for (const auto& step_ptr :
       lazily_test::json_array(lazily_test::json_member(scenario, "steps"))) {
    ...
    cell.merge(static_cast<long>(lazily_test::json_member(step, "merge").as_int()));
...
void test_keyed_reconciliation() {
  ...
  const auto prior = keyed_values(lazily_test::json_member(reconcile_json, "prior"));
  const auto target = keyed_values(lazily_test::json_member(reconcile_json, "target"));
  const auto ops = reconcile(prior, target);
  lazily_test::AssertionKeys expected(std::string(__func__) + " expected",
                                      lazily_test::json_member(*root, "expected"));
  const auto result_order = string_array(*expected.optional("result_order"));
  expected.assert_key_with("ops", [&](const lazily_test::Json& block) {
    ...
    for (std::size_t i = 0; i < ops.size(); ++i) {
      ...
      const auto type = lazily_test::json_string(lazily_test::json_member(want, "type"));
      const auto key = lazily_test::json_string(lazily_test::json_member(want, "key"));
      ...
      if (type == "remove") {
        ...
        assert(ops[i].index == static_cast<std::size_t>(at - result_order.begin()));
        const auto& after = lazily_test::json_string(lazily_test::json_member(want, "after"));
  ...
  for (const auto& entry : prior)
    map.entry(ctx, entry.first, entry.second);
  std::map<std::string, Computed<int>> observers;
  for (const auto& key : string_array(*expected.optional("stable_keys_not_invalidated"))) {
    observers.emplace(key, ctx.computed<int>([&map, key](Compute& compute) {
  ...
  expected.assert_key_with("stable_keys_not_invalidated", [&](const lazily_test::Json& block) {
...
void add_sem_children(SemTree<int, int>& tree, const Json& node) {
  ...
  const auto parent = lazily_test::json_string(lazily_test::json_member(node, "id"));
  const auto order = string_array(lazily_test::json_member(*children, "order"));
  ...
  for (const auto& child_id : order) {
    ...
    tree.add_child(parent, child_id,
                   static_cast<int>(lazily_test::json_member(child, "value").as_int()));
...
void assert_sem_values(SemTree<int, int>& tree, lazily_test::AssertionKeys& expected) {
  for (const auto& key : expected.keys()) {
    if (key == "sibling_a_cached" || key == "downstream_consumer_reran") continue;
...
void test_semtree_incremental() {
  ...
  for (const auto& sv :
    ...
    Context ctx;
    SemTree<int, int> tree(ctx, lazily_test::json_string(lazily_test::json_member(tree_json, "id")),
                           static_cast<int>(lazily_test::json_member(tree_json, "value").as_int()),
                           fold);
    ...
    lazily_test::AssertionKeys expect_after("semtree_incremental.json expect_after",
    ...
    if (expect_after.has("downstream_consumer_reran")) {
      ...
      downstream.emplace(ctx.computed<int>([&, root_handle](Compute& compute) {
        ++downstream_runs;
    ...
    if (edit != nullptr) {
      tree.set_value(lazily_test::json_string(lazily_test::json_member(*edit, "id")),
                     static_cast<int>(lazily_test::json_member(*edit, "value").as_int()));
    } else {
      const auto& remove = lazily_test::json_member(scenario, "remove_child");
      tree.remove_child(lazily_test::json_string(lazily_test::json_member(remove, "parent")),
                        lazily_test::json_string(lazily_test::json_member(remove, "child")));
    }
    ...
    expect_after.assert_key_with_if_present("downstream_consumer_reran", [&](const Json& want) {
...
Block json_block(const Json& value) {
  const auto text = lazily_test::json_string(lazily_test::json_member(value, "text"));
  const Json* anchor = value.find("anchor");
  return anchor == nullptr ? new_block(text)
...
void test_stableid_alignment() {
  ...
  for (const auto& sv :
    ...
    if (scenario.has("blocks")) {
      const auto blocks = json_blocks(lazily_test::json_member(scenario, "blocks"));
      expect.assert_key_with_if_present("key_equal", [&](const Json& equal) {
        for (const auto& pair : lazily_test::json_array(equal)) {
          ...
          if (!block_key_of(blocks[lazily_test::json_u64(*indices[0])])
                   .equals(block_key_of(blocks[lazily_test::json_u64(*indices[1])])))
            return false;
        }
        return true;
      });
      expect.assert_key_with_if_present("key_not_equal", [&](const Json& not_equal) {
    ...
    const auto old_blocks = json_blocks(lazily_test::json_member(scenario, "old"));
    const auto new_blocks = json_blocks(lazily_test::json_member(scenario, "new"));
    if (expect.has("new_key_equals_old_key")) {
      ...
      expect.assert_key_with("new_key_equals_old_key", [&](const Json& pairs) {
        for (const auto& pair : lazily_test::json_array(pairs)) {
          const auto& indices = lazily_test::json_array(*pair);
          if (new_keys[lazily_test::json_u64(*indices[0])] !=
    ...
    expect.assert_key_with_if_present("similarity_min", [&](const Json& similarity_min) {
...
ReplicaMap text_seed(const Json& scenario) {
  ...
  if (replica != nullptr) {
    ReplicaMap result;
    result.emplace(
        "a", TextCrdt::from_str(lazily_test::json_u64(lazily_test::json_member(*replica, "peer")),
                                lazily_test::json_string(seed)));
    return result;
  }
  ReplicaMap result;
  result.emplace(
      "a", TextCrdt::from_str(lazily_test::json_u64(lazily_test::json_member(seed, "peer")),
                              lazily_test::json_string(lazily_test::json_member(seed, "text"))));
  return result;
}

void exchange(ReplicaMap& replicas, const std::string& left_name, const std::string& right_name) {
  ...
  for (const auto& step_ptr : lazily_test::json_array(steps)) {
    ...
    if (step.has("fork")) {
      replicas.emplace(
          ...
          replicas.at("a").fork(lazily_test::json_u64(lazily_test::json_member(step, "peer"))));
    } else if (step.has("clone")) {
      replicas.emplace(
          lazily_test::json_string(lazily_test::json_member(step, "clone")),
          replicas.at(lazily_test::json_string(lazily_test::json_member(step, "from"))).clone());
    } else if (step.has("merge")) {
      const auto& merge = lazily_test::json_member(step, "merge");
      replicas.at(lazily_test::json_string(lazily_test::json_member(merge, "into")))
          .merge(replicas.at(lazily_test::json_string(lazily_test::json_member(merge, "from"))));
    } else if (step.has("exchange")) {
      const auto names = string_array(lazily_test::json_member(step, "exchange"));
      exchange(replicas, names[0], names[1]);
    } else if (step.has("snapshot")) {
      const auto& snapshot = lazily_test::json_member(step, "snapshot");
      const auto from = lazily_test::json_string(lazily_test::json_member(snapshot, "from"));
      const auto into = lazily_test::json_string(lazily_test::json_member(snapshot, "into"));
      TextCrdt target(lazily_test::json_u64(lazily_test::json_member(snapshot, "peer")));
      const bool changed = target.apply_delta(replicas.at(from).delta_since({}));
      assert(changed == lazily_test::json_bool(lazily_test::json_member(step, "expect_changed")));
      replicas.emplace(into, std::move(target));
    } else if (step.has("new")) {
      replicas.emplace(lazily_test::json_string(lazily_test::json_member(step, "new")),
                       TextCrdt(lazily_test::json_u64(lazily_test::json_member(step, "peer"))));
    } else if (step.has("delta")) {
      const auto& delta = lazily_test::json_member(step, "delta");
      auto& into = replicas.at(lazily_test::json_string(lazily_test::json_member(delta, "into")));
      auto& from = replicas.at(lazily_test::json_string(lazily_test::json_member(delta, "from")));
      const bool changed = into.apply_delta(from.delta_since(into.version_vector()));
      assert(changed == lazily_test::json_bool(lazily_test::json_member(step, "expect_changed")));
    } else {
      const auto name =
          step.has("on") ? lazily_test::json_string(lazily_test::json_member(step, "on")) : "a";
      auto& replica = replicas.at(name);
      const auto op = lazily_test::json_string(lazily_test::json_member(step, "op"));
      if (op == "insert") {
        replica.insert(lazily_test::json_u64(lazily_test::json_member(step, "index")),
                       lazily_test::json_string(lazily_test::json_member(step, "ch")));
      } else if (op == "insert_str") {
        replica.insert_str(lazily_test::json_u64(lazily_test::json_member(step, "index")),
                           lazily_test::json_string(lazily_test::json_member(step, "str")));
      } else if (op == "delete") {
        ...
        assert(collected ==
               lazily_test::json_u64(lazily_test::json_member(step, "expect_collected")));
      }
    }
  }
}
...
void assert_text_expectations(const ReplicaMap& replicas, lazily_test::AssertionKeys& expect) {
  expect.assert_key_with_if_present("texts_equal", [&](const Json& equal) {
    for (const auto& pair : lazily_test::json_array(equal)) {
      const auto names = string_array(*pair);
      if (replicas.at(names[0]).text() != replicas.at(names[1]).text()) return false;
    }
    return true;
  });
  expect.assert_key_if_present("text", replicas.at("a").text(), lazily_test::json_string);
  expect.assert_key_if_present("len", replicas.at("a").visible_len(), lazily_test::json_u64);
  expect.assert_key_with_if_present("a_starts_with", [&](const Json& starts) {
    return replicas.at("a").text().rfind(lazily_test::json_string(starts), 0) == 0;
  });
