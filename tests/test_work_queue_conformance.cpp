...
template <typename Queue> struct Readers {
  ...
  Readers(Context &context, Queue &queue)
      : graph(context), handles(queue.reader_handles()) {
    refresh();
  }
  ...
  void assert_invalidates(const Json &expected, const std::string &where) {
    ...
    for (const auto &reader : actual) {
      ...
      REQUIRE(want != nullptr,
              where + ": invalidates." + reader.name + " is absent");
...
void assert_delivery(const WorkQueueDelivery<std::string> &got,
                     const Json &want, const std::string &where) {
  REQUIRE(got.delivery_id ==
              static_cast<std::uint64_t>(want.find("delivery_id")->as_int()),
          where + ": delivery_id mismatch");
  REQUIRE(got.item_id ==
              static_cast<std::uint64_t>(want.find("item_id")->as_int()),
          where + ": item_id mismatch");
  REQUIRE(got.value == want.find("value")->as_str(),
          where + ": delivery value mismatch");
  REQUIRE(got.worker == want.find("worker")->as_str(),
          where + ": delivery worker mismatch");
  REQUIRE(got.attempt ==
              static_cast<std::uint64_t>(want.find("attempt")->as_int()),
          where + ": delivery attempt mismatch");
  REQUIRE(got.deadline ==
              static_cast<std::uint64_t>(want.find("deadline")->as_int()),
...
void assert_state(Queue &queue, const Json &expected,
  ...
  const Json *dead = expected.find("dead_letters");
  REQUIRE(pending && in_flight && dead, where + ": state array is absent");

  const auto got_pending = queue.pending_items();
  REQUIRE(got_pending.size() == pending->array.size(),
          where + ": pending length mismatch");
  for (std::size_t i = 0; i < got_pending.size(); ++i) {
    const Json &want = *pending->array[i];
    REQUIRE(got_pending[i].item_id ==
                static_cast<std::uint64_t>(want.find("item_id")->as_int()),
  ...
  REQUIRE(got_in_flight.size() == in_flight->array.size(),
          where + ": in_flight length mismatch");
  ...
  REQUIRE(got_dead.size() == dead->array.size(),
          where + ": dead-letter length mismatch");
  for (std::size_t i = 0; i < got_dead.size(); ++i) {
    const Json &want = *dead->array[i];
    REQUIRE(got_dead[i].item_id ==
                static_cast<std::uint64_t>(want.find("item_id")->as_int()),
            where + ": dead-letter item_id mismatch");
    REQUIRE(got_dead[i].value == want.find("value")->as_str(),
            where + ": dead-letter value mismatch");
    REQUIRE(got_dead[i].attempts ==
                static_cast<std::uint64_t>(want.find("attempts")->as_int()),
            where + ": dead-letter attempts mismatch");
    const std::string reason =
        got_dead[i].reason == WorkQueueDeadLetterReason::Expired ? "expired"
                                                                 : "nack";
    REQUIRE(reason == want.find("reason")->as_str(),
            where + ": dead-letter reason mismatch");
  }
}
...
std::size_t replay(const std::string &fixture, const std::string &flavor) {
  const auto root = lazily_test::parse_json(
      lazily_test::spec_fixture_text("collections", fixture));
  ...
  REQUIRE(config && steps && !steps->array.empty(),
          fixture + ": config or non-empty steps absent");

  OwnerContext ctx;
  Context &graph = queue_detail::graph(ctx);
  Queue queue(
      ctx,
      static_cast<std::uint64_t>(config->find("visibility_timeout")->as_int()),
  ...
  for (std::size_t i = 0; i < steps->array.size(); ++i) {
    ...
    if (type == "push") {
      ...
      const auto got =
          queue.claim(ctx, op->find("worker")->as_str(),
      ...
      const auto delivery_id =
          static_cast<std::uint64_t>(op->find("delivery_id")->as_int());
      const bool got = type == "ack" ? queue.ack(ctx, worker, delivery_id)
                                     : queue.nack(ctx, worker, delivery_id);
      REQUIRE(got == returns->as_bool(),
              where + ": settlement return mismatch");
    } else if (type == "reap_expired") {
      const auto got = queue.reap_expired(
          ctx, static_cast<std::uint64_t>(op->find("now")->as_int()));
...
int main() {
  ...
  for (const auto &fixture : fixtures) {
    ...
    replayed += replay<ThreadSafeContext, ThreadSafeWorkQueueCell<std::string>>(
        fixture, "thread-safe");
    replayed +=
        replay<AsyncContext, AsyncWorkQueueCell<std::string>>(fixture, "async");
  }
  REQUIRE(replayed > 0, "work-queue replay executed zero steps");
  REQUIRE_FIXTURES_LOADED(2);
  std::cout << "work-queue conformance: 2 canonical fixtures x 3 flavors, "
            << replayed << " steps replayed" << std::endl;
  return 0;
}
