...
void every_flavor_is_defined() {
  ...
  for (const auto &flavor : ledger()) {
    ...
    REQUIRE(defined, std::string("shipped flavor ") + flavor.name +
                         " is not defined in include/lazily");
  }
}

void ledger_is_complete() {
  REQUIRE(ledger().size() == 9,
          "the ledger must cover three primitives x three execution flavors");
}
...
void shipped_flavor_replays_the_corpus() {
  ...
  for (const auto &name : queue_fixtures()) {
    ...
    REQUIRE(!steps->array.empty(),
            name +
                ": fixture has no steps - a vacuous replay would report green");
    ...
    for (std::size_t i = 0; i < steps->array.size(); ++i) {
      ...
      REQUIRE(expected != nullptr,
              name + " step " + std::to_string(i) + ": no expected block");
      if (expected->find("invalidates") != nullptr)
        ++matrices_seen;
    }
  }
...
int main() {
  ...
  std::cout << "queue family ledger: 9 shipped flavors, 11 fixtures read"
            << std::endl;
  return 0;
}
