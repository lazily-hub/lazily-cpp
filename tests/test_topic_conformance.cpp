...
static TopicDurability durability_of(const std::string &s) {
  if (s == "ephemeral")
    return TopicDurability::Ephemeral;
...
template <typename OwnerContext, typename Topic>
  ...
  for (const auto &step : steps->array) {
    ...
    for (const auto &kv : subs->object) {
      ...
      REQUIRE(got.has_value(),
              ("subscription missing: " + tag + " sub=" + kv.first).c_str());
    ...
    for (const auto &id : all_ids)
      if (!expected_ids.count(id))
        REQUIRE(!topic.subscription(id).has_value(),
                ("removed subscription still present: " + tag + " sub=" + id)
                    .c_str());
...
TEST(conformance_topiccell_broadcast_cursor_isolation) {
  run_fixture<Context, TopicCell<std::string>>(
...
int main() {
  ...
  std::cout << "lazily-cpp topic conformance: 4 canonical fixtures x 3 "
               "flavors, "
            << test_passed << "/" << test_count << " groups passed"
            << std::endl;
