...
static std::string fixture_text(const std::string& file) {
  const auto path = std::filesystem::path(__FILE__).parent_path() /
                    "conformance/service" / file;
  std::ifstream input(path);
  REQUIRE(input, "service conformance fixture missing — a conformance test must not pass without its fixture");
...
TEST(test_health) {
  ...
  auto observed = ctx.computed<Health>([hc](Context& c) { return c.get_cell(hc); });
  (void)ctx.get(observed);
...
TEST(test_readiness) {
  ...
  auto observed = ctx.computed<bool>([rc](Context& c) { return c.get_cell(rc); });
  (void)ctx.get(observed);
...
TEST(test_discovery) {
  ...
  auto observed = ctx.computed<Map>([dc](Context& c) { return c.get_cell(dc); });
  (void)ctx.get(observed);
...
TEST(test_service_registry) {
  ...
  auto observed = ctx.computed<Map>([pc](Context& c) { return c.get_cell(pc); });
  (void)ctx.get(observed);
