...
Overflow overflow_of(const std::string &text) {
  ...
  if (text == "drop_newest")
    return Overflow::DropNewest;
  if (text == "drop_oldest")
    return Overflow::DropOldest;
  if (text == "conflate")
    return Overflow::Conflate;
  if (text == "spill")
    return Overflow::Spill;
  REQUIRE(false, "unknown overflow `" + text + "`");
  return Overflow::Conflate;
}

IngressTransportKind transport_of(const std::string &text) {
  if (text == "event_channel")
    return IngressTransportKind::EventChannel;
  if (text == "rpc_triggered")
    return IngressTransportKind::RpcTriggered;
  if (text == "bounded_polling")
    return IngressTransportKind::BoundedPolling;
  REQUIRE(false, "unknown transport `" + text + "`");
  return IngressTransportKind::EventChannel;
}

IngressError error_of(const std::string &text) {
  if (text == "transport_closed")
    return IngressError::TransportClosed;
  if (text == "decode_failed")
    return IngressError::DecodeFailed;
  if (text == "authority_lost")
    return IngressError::AuthorityLost;
  REQUIRE(false, "unknown error `" + text + "`");
  return IngressError::TransportClosed;
}

IngressDropReason drop_reason_of(const std::string &text) {
  if (text == "stale_generation")
    return IngressDropReason::StaleGeneration;
  if (text == "duplicate_sequence")
    return IngressDropReason::DuplicateSequence;
  if (text == "duplicate_buffered")
    return IngressDropReason::DuplicateBuffered;
  if (text == "reorder_window_overflow")
    return IngressDropReason::ReorderWindowOverflow;
  if (text == "expired")
    return IngressDropReason::Expired;
  if (text == "backpressure")
    return IngressDropReason::Backpressure;
  if (text == "scope_closed")
    return IngressDropReason::ScopeClosed;
...
IngressLifecycle lifecycle_of(const std::string &text) {
  ...
  if (text == "live")
    return IngressLifecycle::Live;
  if (text == "suspended")
    return IngressLifecycle::Suspended;
  if (text == "closed")
    return IngressLifecycle::Closed;
...
IngressReadiness readiness_of(const std::string &text) {
  ...
  if (text == "ready")
    return IngressReadiness::Ready;
  if (text == "stale")
    return IngressReadiness::Stale;
  if (text == "suspended")
...
IngressPolicy policy_of(const Json &value, const std::string &where) {
  ...
  policy.freshness_horizon =
      json_u64(member(value, "freshness_horizon", where));
  ...
  policy.receipt_capacity = static_cast<std::size_t>(
      json_u64(member(value, "receipt_capacity", where)));
  policy.retry_base = json_u64(member(value, "retry_base", where));
  policy.retry_ceiling = json_u64(member(value, "retry_ceiling", where));
  return policy;
}

IngressAdmission expected_admission(const Json &value,
                                    const std::string &where) {
  const std::string kind = json_string(member(value, "admission", where));
  if (kind == "accepted")
    return IngressAdmission::accepted(
        json_u64(member(value, "delivered_through", where)));
  if (kind == "conflated")
    return IngressAdmission::conflated(
        json_u64(member(value, "delivered_through", where)));
  if (kind == "buffered")
    return IngressAdmission::buffered(
        json_u64(member(value, "gap_from", where)));
  if (kind == "generation_handoff")
    return IngressAdmission::generation_handoff(
        json_u64(member(value, "from", where)),
        json_u64(member(value, "to", where)));
  if (kind == "dropped")
    return IngressAdmission::dropped(
        drop_reason_of(json_string(member(value, "reason", where))));
  if (kind == "blocked")
...
std::optional<ReplayRequest> expected_replay(const Json &value,
                                             const std::string &where) {
  if (value.is_null())
    return std::nullopt;
...
template <typename Cell, typename Cx> struct Model {
  ...
  Model(const IngressPolicy &policy, IngressTransportKind transport,
        std::uint64_t poll_interval)
      : ctx(), cell(ctx, policy, transport, poll_interval) {}

  void open(const Key &key, std::uint64_t generation) {
    cell.open(ctx, key, generation);
  }
  IngressAdmission admit(Env envelope) {
    return cell.admit(ctx, std::move(envelope));
  }
  std::optional<ReplayRequest> suspend(const Key &key) {
    return cell.suspend(ctx, key);
  }
  ...
  IngressReadiness readiness(const Key &key) {
    return cell.readiness(ctx, key);
  }
  std::optional<IngressAuthority> authority(const Key &key) {
    return cell.authority(ctx, key);
  }
  std::optional<IngressRetry> retry(const Key &key) {
    return cell.retry(ctx, key);
  }
  ...
  bool readiness_is_valid(const Key &key) {
    return lazily::ingress_detail::is_valid(ctx,
                                            cell.readiness_handle(ctx, key));
  }
  bool authority_is_valid(const Key &key) {
    return lazily::ingress_detail::is_valid(ctx,
                                            cell.authority_handle(ctx, key));
  }
  bool retry_is_valid(const Key &key) {
    return lazily::ingress_detail::is_valid(ctx, cell.retry_handle(ctx, key));
  }
  bool accepted_is_valid() {
    return lazily::ingress_detail::is_valid(ctx, cell.accepted_handle());
  }
  bool dropped_is_valid() {
    return lazily::ingress_detail::is_valid(ctx, cell.dropped_handle());
  }
  bool errors_is_valid() {
    return lazily::ingress_detail::is_valid(ctx, cell.errors_handle());
  }

  std::optional<IngressScopeView> view(const Key &key) {
    return cell.view(key);
  }
};
...
template <typename M>
ValiditySnapshot snapshot_validity(M &model, const std::vector<Key> &keys) {
  ValiditySnapshot snapshot;
  for (const auto &key : keys)
    snapshot.scopes[key] = {model.value_is_valid(key),
...
void assert_state(M &model, lazily_test::AssertionKeys &expected,
  ...
  for (const auto &entry : scopes.object) {
    ...
    REQUIRE(view->lifecycle ==
                lifecycle_of(json_string(member(want, "lifecycle", where))),
            where + ": " + key + " lifecycle");
    ...
    REQUIRE(view->delivered_through ==
            ...
            where + ": " + key + " watermark");
    REQUIRE(view->buffered ==
                static_cast<std::size_t>(json_u64(member(want, "buffered", where))),
            where + ": " + key + " buffered");
    ...
    REQUIRE(model.value(key) ==
                json_optional_u64(member(want, "window", where)),
            where + ": " + key + " window");
    REQUIRE(model.readiness(key) ==
                readiness_of(json_string(member(want, "readiness", where))),
            where + ": " + key + " readiness");
    ...
    if (want_authority.is_null()) {
      REQUIRE(!authority.has_value(),
              where + ": " + key + " must claim no authority");
    } else {
      REQUIRE(authority.has_value(), where + ": " + key + " authority absent");
      REQUIRE(authority->generation ==
                  json_u64(member(want_authority, "generation", where)),
              where + ": " + key + " authority generation");
      REQUIRE(authority->delivered_through ==
                  json_optional_u64(
                      member(want_authority, "delivered_through", where)),
              where + ": " + key + " authority watermark");
      REQUIRE(authority->stamped_at ==
                  json_u64(member(want_authority, "stamped_at", where)),
    ...
    if (want_retry.is_null()) {
      ...
      REQUIRE(retry->resume_from ==
                  json_u64(member(want_retry, "resume_from", where)),
...
void assert_invalidation(lazily_test::AssertionKeys &expected,
  ...
  REQUIRE(want_scopes.is_object(),
          where + ": invalidates.scopes is not an object");
  for (const auto &entry : want_scopes.object) {
    ...
    for (std::size_t slot = 0; slot < 4; ++slot) {
      const bool expected =
          json_bool(member(*entry.second, kKinds[slot], where));
      const bool invalidated =
          before_it->second[slot] && !after_it->second[slot];
      REQUIRE(invalidated == expected,
              where + ": " + key + "." + kKinds[slot] +
                  " invalidation mismatch (was valid=" +
                  std::to_string(before_it->second[slot]) + ", now valid=" +
                  std::to_string(after_it->second[slot]) + ", expected " +
                  (expected ? "true" : "false") + ")");
    }
  }
  ...
  for (std::size_t slot = 0; slot < 3; ++slot) {
    const bool expected =
        json_bool(member(want_receipts, kChannels[slot], where));
    const bool invalidated = before.receipts[slot] && !after.receipts[slot];
    REQUIRE(invalidated == expected,
            where + ": receipts." + kChannels[slot] +
...
template <typename M>
std::size_t replay(const Json &fixture, const std::string &label) {
  ...
  const std::uint64_t poll_interval =
      json_u64(member(fixture, "poll_interval", label));
  ...
  for (const auto &step_ptr : steps.array) {
    ...
    if (op != nullptr) {
      ...
      if (key != nullptr && key->type == Json::Type::String)
        note(key->as_str());
    }
  ...
  for (std::size_t index = 0; index < steps.array.size(); ++index) {
    ...
    if (type == "admit") {
      ...
      if (returns != nullptr)
        REQUIRE(admission == expected_admission(*returns, where),
                where + ": admission");
    } else if (type == "open") {
      model.open(json_string(member(op, "key", where)),
                 json_u64(member(op, "generation", where)));
    } else if (type == "drain") {
      ...
      const ReplayRequest request =
          model.reconnect(json_string(member(op, "key", where)),
                          json_u64(member(op, "generation", where)));
      if (returns != nullptr) {
        const auto expected =
            expected_replay(member(*returns, "replay", where), where);
        REQUIRE(expected.has_value() && *expected == request,
                where + ": replay request");
      }
    } else if (type == "close") {
...
void every_flavour_is_defined(const std::map<std::string, std::size_t> &replayed) {
  ...
  REQUIRE(!sources.empty(),
          "read no headers from include/lazily; the ledger check would be "
          "vacuous");
  for (const auto &row : ledger()) {
    const bool defined = sources.find(row.marker_type) != std::string::npos;
    REQUIRE(defined == row.shipped,
            std::string("ledger row `") + row.flavour + "` claims shipped=" +
                (row.shipped ? "true" : "false") + " but `" + row.marker_type +
...
void corpus_is_present_and_non_trivial() {
  ...
  REQUIRE(total >= 30, "the ingress corpus replays only " +
                           std::to_string(total) +
...
int main() {
  ...
  REQUIRE(replayed["single-threaded"] == total &&
              replayed["single-threaded"] > 0,
          "every corpus step must run against the single-threaded flavour");

  replayed["thread-safe"] = replay_corpus<
      Model<ThreadSafeIngressCell<Key, Payload, Sum>, ThreadSafeContext>,
  ...
  replayed["async"] = replay_corpus<
      Model<AsyncIngressCell<Key, Payload, Sum>, AsyncContext>,
  ...
  std::cout << "ingress family: 3 shipped flavours x " << fixtures().size()
            << " fixtures, " << total << " steps each (" << (total * 3)
            << " step replays)" << std::endl;
  return 0;
}
